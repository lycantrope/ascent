...
class LocalFeatureNet(nn.Module):
    def __init__(self, patch_size_xy: int, patch_size_z: int):
        ...
        self.patch_size_xy = torch.tensor(patch_size_xy, dtype=torch.long)
        self.patch_size_z = torch.tensor(patch_size_z, dtype=torch.long)
        self.z_offsets = [0]
        for i in range(1, patch_size_z // 2 + 1):
            ...
            if len(self.z_offsets) < patch_size_z:
                self.z_offsets.append(-i)
        self.z_offsets = torch.tensor(self.z_offsets, dtype=torch.long)
    ...
    def get_patches_vectorized(self, x: OEDItem):
        ...
        base_grid_xy = torch.stack([grid_x, grid_y], axis=-1)  # (patch_size, patch_size, 2)
        ...
        grid_z = torch.stack(grid_z, axis=0).unsqueeze(
            -1
        )  # (len(z_offsets), patch_size, patch_size, 1)
        base_grid_xyz = torch.cat(
            [base_grid_xy, grid_z], axis=-1
        ...
        center_positions = torch.empty((S, 1, 1, 1, 3), dtype=torch.float, device=image.device)
        for i in range(N):
            # zyx -> xyz
            center_positions[:, 0, 0, 0, 0] = object_positions[i, :, 2] / (W - 1) * 2 - 1
            center_positions[:, 0, 0, 0, 1] = object_positions[i, :, 1] / (H - 1) * 2 - 1
            center_positions[:, 0, 0, 0, 2] = object_positions[i, :, 0] / (D - 1) * 2 - 1
            ...
            input = image[i : i + 1].expand(S, -1, -1, -1, -1)  # (1, C, D, H, W) -> (S, C, D, H, W)

            # Get patches
            patches[i] = nn.functional.grid_sample(
                input,
                sampling_grid,
                mode="nearest",
                padding_mode="zeros",
                align_corners=True,  # (S, C, len(z_offsets), patch_size, patch_size)
            )[:, 0]  # only use the first channel
    ...
    def get_patches(self, x: OEDItem):
        ...
        for i in range(x["image"].shape[0]):
            ...
            min_x = torch.round(object_positions[~mask[i], 2].min()).int().item()
            max_x = torch.round(object_positions[~mask[i], 2].max()).int().item()
            min_y = torch.round(object_positions[~mask[i], 1].min()).int().item()
            max_y = torch.round(object_positions[~mask[i], 1].max()).int().item()
            pad_x_left = max(self.patch_size_xy // 2 - min_x, 0)
            pad_x_right = max(self.patch_size_xy // 2 - (image_zyx.shape[-1] - max_x), 0)
            pad_y_top = max(self.patch_size_xy // 2 - min_y, 0)
            pad_y_bottom = max(self.patch_size_xy // 2 - (image_zyx.shape[-2] - max_y), 0)
                ...
                if mask[i, j]:
                    ...
                    for z_offset in self.z_offsets:
                        ...
                        if cur_z < 0 or cur_z >= image_zyx.shape[-3]:
                            cur_patch = torch.zeros(
                                [self.patch_size_xy, self.patch_size_xy],
                                dtype=image_zyx.dtype,
                                device=image_zyx.device,
                            )
                        else:
                            cur_patch = image_zyx[
                                cur_z,
                                pad_y_top + pos_y - self.patch_size_xy // 2 : pad_y_top
                                + pos_y
                                + self.patch_size_xy // 2,
                                pad_x_left + pos_x - self.patch_size_xy // 2 : pad_x_left
                                + pos_x
                                + self.patch_size_xy // 2,
                            ]
                        cur_patches.append(cur_patch)
                    cur_patches = torch.stack(cur_patches, axis=0)
                    list_patches.append(cur_patches)

            list_patches = torch.stack(list_patches, axis=0).to(device)
...
class LocalFeatureViT(LocalFeatureNet):
    def __init__(self, patch_size_xy: int, patch_size_z: int, pretrained: str):
        ...
        self.extra_tokens = {"channels": torch.tensor([range(patch_size_z)], dtype=torch.long)}
        self.backbone = torch.hub.load("insitro/ChannelViT", pretrained, pretrained=True)
        if patch_size_z != self.backbone.in_chans:
            self.backbone = modify_channelvit_input_channels(self.backbone, patch_size_z)
...
class LocalFeatureCNN(LocalFeatureNet):
    def __init__(
        self, patch_size_xy: int, patch_size_z: int, out_features: int, backbone: str, weights: str
    ):
        ...
        if "resnet" in backbone:
            ...
            self.backbone.conv1 = nn.Conv2d(
                patch_size_z, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False
            )
            self.backbone.fc = nn.Linear(
                in_features=self.backbone.fc.in_features, out_features=out_features, bias=True
            )
        elif "mobilenet" in backbone:
            if backbone == "mobilenet_v2":
                self.backbone = models.mobilenet_v2(weights=weights)
                self.backbone.features[0][0] = nn.Conv2d(
                    patch_size_z, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False
                )
                ...
                if backbone == "mobilenet_v3_small":
                    ...
                    raise Exception("Invalid model name")
                self.backbone.features[0][0] = nn.Conv2d(
                    patch_size_z, 16, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False
                )
                self.backbone.classifier = nn.Linear(
...
class NETr(nn.Module):
    ...
    def __init__(
        ...
        if use_local_features:
            self.lf_module = LocalFeatureViT(lf_patch_size_xy, lf_patch_size_z, lf_pretrained)
            if lf_weights is not None and len(lf_weights) > 0 and lf_weights != "None":
                ...
                state_dict = {k.replace("chvit.", "backbone."): v for k, v in state_dict.items()}
    ...
    def forward(self, x: dict):
        ...
        if self.use_local_features:
            ...
            local_features = local_features.view(-1, local_features.shape[-1])  # (NS, C_lf)
        ...
        if self.use_positional_encoding:
            ...
            if self.pe_scaling == "physical":
                center = torch.tensor([0, 0, 0], dtype=torch.float, device=device)  # (3,)
                scaling = x["spacing"]  # (N, 3)
            elif self.pe_scaling == "relative":
                center = torch.tensor(
                    [D / 2.0, H / 2.0, W / 2.0], dtype=torch.float, device=device
                )  # (3,)
                scale_x = torch.tensor(2 / x["image"].shape[-1], dtype=torch.float, device=device)
                scale_y = torch.tensor(2 / x["image"].shape[-2], dtype=torch.float, device=device)
                space_xy = torch.mean(x["spacing"][..., 1:3], dim=-1)  # (N,)
                scale_z = (scale_x + scale_y) / 2 * x["spacing"][..., 0] / space_xy  # (N,)
                scaling = torch.stack(
            ...
            for i in range(N):
                pos_encoding.append((x["coords"][i] - center[None, :]) * scaling[i][None, :])
            pos_encoding = torch.stack(pos_encoding, axis=0)  # (N, S, 3)
        ...
        # Transformer encoder
        if self.use_local_features and self.use_positional_encoding:
            neuron_features = local_features + pos_encoding  # (N, S, D)
        elif self.use_local_features:
            neuron_features = local_features
        elif self.use_positional_encoding:
            neuron_features = pos_encoding
        else:
            raise Exception("At least one of local features or positional encoding should be used.")
        ...
        if self.use_transformer:
            ...
            matching_features = self.encoder(neuron_features, src_key_padding_mask=key_padding_mask)
